...
#-------------------Funcion decoradora---------------
def conectar_y_mensajes(funcionBBDD):
	def funcion_interna():
		pass
...
def  leerBBDD():
	...
	idRead=idPantalla.get()

	miConexion=sqlite3.connect("Usuarios")

	miCursor=miConexion.cursor()

	miCursor.execute("SELECT * FROM USUARIOS WHERE ID =" + str(idRead))# primera forma de usar el idread
